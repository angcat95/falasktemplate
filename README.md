...
- 🎯 Entry points to execute your program using `python -m <falasktemplate>` or `$ falasktemplate` with basic CLI argument parsing.
...
---
# falasktemplate Flask Application

Awesome falasktemplate created by angcat95
...
```bash
git clone https://github.com/angcat95/falasktemplate falasktemplate
cd falasktemplate
make install
```
...
```bash
falasktemplate create-db   # run once
falasktemplate populate-db  # run once (optional)
falasktemplate add-user -u admin -p 1234  # ads a user
falasktemplate run
```
