...
original_author="angcat95"
original_name="falasktemplate"
original_urlname="falasktemplate"
original_description="Awesome falasktemplate created by angcat95" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/falasktemplate "${name}"
...
then
    ...
    echo "then run 'falasktemplate' or 'python -m falasktemplate'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'falasktemplate' or 'python -m falasktemplate' or 'poetry run falasktemplate'"
fi
